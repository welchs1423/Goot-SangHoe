...
<body>
	...
	<h2>보드카</h2><br>
	    ...
	    Total  ${vodkacri.total}건 <c:out value="${vodkacri.cri.pageNum}"/>페이지
		<ul id="gall_ul">
		
        	
			<c:forEach var="product" items="${vodkaList}">
			<li class="gall_li ">
            	<ul class="gall_con">
                	<li class="gall_href">
		...
		<div class="pull-right">
						<ul class="pagination">
							<c:if test="${vodkacri.prev }">
								<li class="paginate_button previous">
									<a href="${vodkacri.startPage-1 }">&lt;</a>
								</li>
							</c:if>
							<c:forEach var="num" begin="${vodkacri.startPage }" end="${vodkacri.endPage }" step="1">
								<li class="paginate_button ${vodkacri.cri.pageNum == num ? 'active':''}">
									<a href="${num }">${num }</a>
								</li>
							</c:forEach>
							<c:if test="${vodkacri.next }">
								<li class="paginate_button">
									<a href="${vodkacri.endPage+1 }">&gt;</a>
								</li>
							</c:if>
						</ul>
					</div>
