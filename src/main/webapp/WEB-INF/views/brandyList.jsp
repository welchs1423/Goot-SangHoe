...
<body>
	...
	<h2>브랜디</h2><br>
	    ...
	    Total  ${brandycri.total}건 <c:out value="${brandycri.cri.pageNum}"/>페이지
		<ul id="gall_ul">
		
        	
			<c:forEach var="product" items="${brandyList}">
			<li class="gall_li ">
            	<ul class="gall_con">
                	<li class="gall_href">
		...
		<div class="pull-right">
						<ul class="pagination">
							<c:if test="${brandycri.prev }">
								<li class="paginate_button previous">
									<a href="${brandycri.startPage-1 }">&lt;</a>
								</li>
							</c:if>
							<c:forEach var="num" begin="${brandycri.startPage }" end="${brandycri.endPage }" step="1">
								<li class="paginate_button ${brandycri.cri.pageNum == num ? 'active':''}">
									<a href="${num }">${num }</a>
								</li>
							</c:forEach>
							<c:if test="${brandycri.next }">
								<li class="paginate_button">
									<a href="${brandycri.endPage+1 }">&gt;</a>
								</li>
							</c:if>
						</ul>
					</div>
