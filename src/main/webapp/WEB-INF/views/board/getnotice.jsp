...
<form>
    ...
    <header>
        <h1 id="bo_v_title">${getnotice.title }</h1>
    </header>

    <section id="bo_v_info">
        <h2>페이지 정보</h2>
        작성자 <strong><span class="sv_member">최고관리자</span></strong>
        <span class="sound_only">작성일</span><strong>${getnotice.regdate }</strong>
        조회<strong>${getnotice.viewcnt }회</strong>
    </section>
    ...
    <section id="bo_v_atc">
        ...
        <!-- 본문 내용 시작 { -->
        <div id="bo_v_con">
        <p><b><span style="font-size:18pt;">${getnotice.content }</span></b></p></div>
                <!-- } 본문 내용 끝 -->
    ...
    <a id="modifyBtn" href=""><button type="button" class="btn btn-primary" data-oper="modify" style="float:right;">수정</button></a>
    
   


</form>
<form action="/board/removenotice" method="post">
	        		<input type="hidden" name="pageNum" value="${cri.pageNum }">
	        		<input type="hidden" name="amount" value="${cri.amount }">
	        		<input type="hidden" name="${_csrf.parameterName }" value="${_csrf.token}">
	        		<input type="hidden" name="nno" value="${getnotice.nno}">
	        		<a id="removeBtn" href=""><button type="button" class="btn btn-danger" data-oper="remove" style="float:right;">삭제</button></a>
	        	</form>
</article>
...
<script>
	$(function(){
		var nno = "${getnotice.nno}";
		var amount = "${cri.amount}";
		
		$("#modifyBtn").click(function(e){	//	새 게시글 등록 버튼
			e.preventDefault();
			location.href="/board/modifynotice?nno=" + nno;
		});
