...
<body>
	...
	<h2>리큐르</h2><br>
	    ...
	    Total  ${liquorcri.total}건 <c:out value="${liquorcri.cri.pageNum}"/>페이지
		<ul id="gall_ul">
		
        	
			<c:forEach var="product" items="${liquorList}">
			<li class="gall_li ">
            	<ul class="gall_con">
                	<li class="gall_href">
		...
		<div class="pull-right">
						<ul class="pagination">
							<c:if test="${liquorcri.prev }">
								<li class="paginate_button previous">
									<a href="${liquorcri.startPage-1 }">&lt;</a>
								</li>
							</c:if>
							<c:forEach var="num" begin="${liquorcri.startPage }" end="${liquorcri.endPage }" step="1">
								<li class="paginate_button ${liquorcri.cri.pageNum == num ? 'active':''}">
									<a href="${num }">${num }</a>
								</li>
							</c:forEach>
							<c:if test="${liquorcri.next }">
								<li class="paginate_button">
									<a href="${liquorcri.endPage+1 }">&gt;</a>
								</li>
							</c:if>
						</ul>
					</div>
